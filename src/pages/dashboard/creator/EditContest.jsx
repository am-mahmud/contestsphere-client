...
const EditContest = () => {
  ...
  useEffect(() => {
    const fetchContest = async () => {
      try {
        ...
        reset(contest);
        setDeadline(contest.deadline.slice(0, 16));
        setLoading(false);
      } catch (err) {
        Swal.fire({ icon: 'error', title: 'Error loading contest' });
        setLoading(false);
      }
    };
    fetchContest();
  ...
  const onSubmit = async (data) => {
    try {
      setSubmitting(true);
      await contestAPI.updateContest(id, {
        ...data,
        deadline,
        price: parseFloat(data.price),
        prizeMoney: parseFloat(data.prizeMoney),
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-3xl font-bold mb-2">Edit Contest</h1>
      <p className="text-gray-600 mb-6">Update contest details</p>

      <form onSubmit={handleSubmit(onSubmit)} className="card bg-white shadow-lg">
        <div className="card-body space-y-4">
          <div className="form-control">
            <label className="label"><span className="font-semibold">Name *</span></label>
            <input type="text" className="input input-bordered" {...register('name', { required: 'Required', minLength: { value: 5, message: 'Min 5 chars' } })} />
            {errors.name && <span className="text-error text-sm">{errors.name.message}</span>}
          </div>
          ...
          <div className="form-control">
            ...
            <input type="url" className="input input-bordered" {...register('image', { required: 'Required', pattern: { value: /^https?:\/\/.+\..+/i, message: 'Invalid URL' } })} />
            {errors.image && <span className="text-error text-sm">{errors.image.message}</span>}
          </div>
          ...
          <div className="form-control">
            ...
            <textarea className="textarea textarea-bordered h-20" {...register('description', { required: 'Required', minLength: { value: 50, message: 'Min 50 chars' } })}></textarea>
            {errors.description && <span className="text-error text-sm">{errors.description.message}</span>}
          </div>
          ...
          <div className="form-control">
            ...
            <textarea className="textarea textarea-bordered h-20" {...register('taskInstruction', { required: 'Required', minLength: { value: 30, message: 'Min 30 chars' } })}></textarea>
            {errors.taskInstruction && <span className="text-error text-sm">{errors.taskInstruction.message}</span>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="form-control">
              ...
              <input type="number" step="0.01" className="input input-bordered" {...register('price', { required: 'Required', min: { value: 1, message: 'Min $1' } })} />
              {errors.price && <span className="text-error text-sm">{errors.price.message}</span>}
            </div>
            <div className="form-control">
              <label className="label"><span className="font-semibold">Prize ($) *</span></label>
              <input type="number" step="0.01" className="input input-bordered" {...register('prizeMoney', { required: 'Required', min: { value: 1, message: 'Min $1' } })} />
              {errors.prizeMoney && <span className="text-error text-sm">{errors.prizeMoney.message}</span>}
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            <button type="submit" className="btn bg-[#20beff] text-white flex-1" disabled={submitting}>
