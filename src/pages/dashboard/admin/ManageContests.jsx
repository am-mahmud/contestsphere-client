...
const ManageContests = () => {
  ...
  const updateStatus = async (id, status, name) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.put(`/api/contests/${id}/status`, { status });
          setContests(contests.map(c => c._id === id ? { ...c, status } : c));
            ...
            <tbody>
              {contests.map(c => (
                <tr key={c._id}>
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs opacity-60">{c.description.slice(0, 30)}...</div>
                      </div>
                    </div>
                  </td>
                  <td>
                    {c.creatorId ? (
                      <div className="flex items-center gap-2">
                        <img src={c.creatorId.photo} alt="" className="w-8 h-8 rounded-full" />
                  ...
                  <td><span className="badge badge-ghost text-xs">{c.contestType}</span></td>
                  <td><span className={`badge ${badges[c.status]} text-xs`}>{c.status}</span></td>
                  <td className="font-semibold text-[#20beff]">${c.prizeMoney}</td>
                  <td className="text-center">{c.participantCount}</td>
                  <td className="text-sm">{new Date(c.deadline).toLocaleDateString()}</td>
                  <td>
                    <div className="flex gap-1">
                      {c.status === 'pending' && (
                        <>
                          <button onClick={() => updateStatus(c._id, 'confirmed', c.name)} className="btn btn-sm btn-ghost text-green-600" title="Approve">
                            <FaCheck />
                          </button>
                          <button onClick={() => updateStatus(c._id, 'rejected', c.name)} className="btn btn-sm btn-ghost text-yellow-600" title="Reject">
                            <FaTimes />
                          </button>
                        </>
                      )}
                      <button onClick={() => deleteContest(c._id, c.name)} className="btn btn-sm btn-ghost text-red-600" title="Delete">
                        <FaTrash />
                      </button>
                    </div>
                  </td>
