...
const WinnerAdvertisement = () => {
  const { data: recentWinners } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/api/contests/winners/all?limit=20');
  ...
  const totalPrizeMoney = recentWinners?.reduce((sum, c) => sum + (c.prizeMoney || 0), 0) || 0;
                ...
                <div className="p-6">
                  {contest.winnerId && (
                    <div className="flex items-center gap-3 mb-4">
                      <img
                        src={contest.winnerId.photo || '/default-avatar.png'}
